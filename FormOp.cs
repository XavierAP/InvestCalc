...
namespace JP.InvestCalc
{
	...
	internal partial class FormOp :Form
	{
		...
		private void ConfirmClose(object sender, FormClosingEventArgs ea)
		{
			...
			{
				Debug.Assert(portfolio.ContainsKey(txtStock.Text));
				var max = (decimal)portfolio[txtStock.Text];
		...
		private bool HasError()
		{
			...
			{
				MessageBox.Show(this, $"You must select{(txtStock.Enabled ? " or enter" : null)} a stock name.", Program.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
				listStocks.Focus();
				return true;
			}
			if(opCurrent.SharesChange && numShares.Value == 0)
			{
				MessageBox.Show(this, "You must enter an amount of shares.", Program.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
				numShares.Focus();
				return true;
			}
			if(!opCurrent.SharesChange && numTotal.Value == 0)
			{
				MessageBox.Show(this, "You must enter a money amount.", Program.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
				numTotal.Focus();
				return true;
			}
			return false;
